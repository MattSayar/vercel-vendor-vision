        ...
        <div className="border-b border-border p-4">
          ...
          <div className="relative mt-3">
            ...
            <input
              ...
              className="h-7 w-full rounded-md border border-input bg-background pl-7 pr-7 text-base text-foreground placeholder:text-muted-foreground focus:border-ring focus:outline-none"
            />
            {searchQuery && (
          ...
          <div className="flex flex-col">
            {filteredCases.map((c) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <div className="flex size-6 items-center justify-center rounded-md bg-muted text-sm font-bold text-muted-foreground">
                    {c.vendorName.charAt(0)}
                  </div>
                  <span className="text-base font-semibold text-foreground">{c.vendorName}</span>
                </div>
                <p className="text-sm leading-relaxed text-muted-foreground line-clamp-2">{c.summary}</p>
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3">
                <h2 className="text-xl font-bold text-foreground">{selectedCase.vendorName}</h2>
                <span className={cn("inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-sm font-semibold", getSeverityColor(selectedCase.severity))}>
          ...
          <div className="mt-6 rounded-lg border border-border bg-card p-5">
              ...
              {selectedCase.recommendedActions.map((action) => {
                ...
                return (
                  <div key={action.id} className="flex items-center gap-3 rounded-md border border-border p-3">
                    ...
                    />
                    <span className={cn("text-base text-foreground/80 flex-1", allApproved && checked && "line-through opacity-60")}>
                      {action.label}
                    </span>
                    {allApproved && checked && (
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              {!allApproved && (
                <button className="rounded-md border border-input px-4 py-2 text-base font-semibold text-foreground hover:bg-muted transition-colors">
                  Customize & Execute
                </button>
              )}
          ...
          <div className="mt-6 rounded-lg border-l-4 border-l-primary border border-primary/10 bg-primary/[0.03] p-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-semibold text-foreground">AI Investigation Summary</h3>
            </div>
            <p className="mt-3 text-base leading-relaxed text-foreground/80">{selectedCase.aiSummary}</p>
          </div>

          {/* Evidence Timeline */}
          <div className="mt-6">
            <h3 className="text-base font-semibold text-foreground">Evidence Timeline</h3>
              ...
              {selectedCase.evidence.map((ev, i) => (
                <div key={i} className="flex gap-3 rounded-lg border border-border bg-card p-3.5">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className={cn("text-sm font-semibold", ev.sourceColor)}>
                        {ev.source}
                      </Badge>
                      <span className="text-sm text-muted-foreground">{ev.timestamp}</span>
                      <span className={cn("ml-auto text-sm font-semibold", getConfidenceColor(ev.confidence))}>
                        {ev.confidence}% confidence
                      </span>
                    </div>
                    <p className="mt-1.5 text-base leading-relaxed text-foreground/80">{ev.description}</p>
                  </div>
                </div>
              ))}
            </div>
